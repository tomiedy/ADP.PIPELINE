...
$(document).ready(function () {
    ...
    $("#btnRemoveImgEktp").click(function (e) {
        e.preventDefault();
        $("#fileSkNonEKtp").val("");
        $("#imgnonektp").attr("src", "/Content/images/blank-document.jpg");
    });

    $("#btnRemoveImgSipt").click(function (e) {
        e.preventDefault();
        $("#fileSipt").val("");
        $("#imgsipt").attr("src", "/Content/images/blank-document.jpg");
    });
    ...
    //Insert Permohonan Keterangan NJOP
    $("#saveKeteranganNjop").click(function () {
        debugger;
...
function Simpan() {
    ...
    var masaPajak2 = $("#masaPajak2").val();
    var keperluan = $("#keperluan").val();
    var ktp = $("#fileKtp").val();
    var sipt = $("#fileSipt").val();
    var skNonEktp = $("#fileSkNonEKtp").val();
    $.ajax({
        url: '/KeteranganNjop/InsertKeteranganNjop',
        data: {
            nop: nop,
            noKtp: noKtp,
            namaKtp: namaKtp,
...
// function preview image SIPT
$("#fileSipt").change(function () {
    readImageSipt(this);
});

var targetResultSipt = "";
function readImageSipt(input) {
    if (input.files && input.files[0]) {
        if (ValidateMaxFileSize(input)) {
            var reader = new FileReader();
            reader.onload = function (e) {

                $("#imgsipt").attr('src', e.target.result);
                $("#asipt").attr('href', e.target.result);

                targetResultSipt = e.target.result;
            }
            reader.readAsDataURL(input.files[0]);
        }
        else {
